# SportsData_Portfolio

このリポジトリは、スポーツデータ分析を通じてPythonやデータ分析のスキルを高めるための学習記録です。  
Jリーグ観戦予測や野球データの写経を中心に、転職に向けたポートフォリオ構築を目指しています。

## 学習目的
- Pythonの基礎力と応用力の習得
- GitHubを活用した成果の可視化
- データエンジニア転職に向けた実践的な準備

## 実施記録
- 
### 🗓️ 2025年10月12日：初めてのpybaseball写経 
### 📚 参考文献
### copilotにて 実施内容はREADME（MarkDown)作成

このプロジェクトでは、以下の書籍を参考にしています：

- **『野球データでやさしく学べるPython入門』斎藤 週 著**
- ## 参考文献

- 『野球データでやさしく学べるPython入門』斎藤 週 著  
　野球の実データを題材に、Pythonによるデータ分析の基礎を学べる入門書。  
　可視化や統計処理の実践的な手法が丁寧に解説されており、本プロジェクトのグラフ作成やデータ理解に活用しています。
- `!pip install pybaseball` を実行し、ライブラリを導入
- `statcast_pitcher` を使って、大谷翔平投手の2022年の投球データを取得
- `value_counts()` と `plot.pie()` を使って球種の割合を円グラフで可視化
- Google Colabで写経し、GitHubにノートブックをアップロード
- Markdownの編集やREADMEの構成にも初挑戦


#📊 可視化コンテンツの追加（2025年10月15日）
本日、新たに以下の３種のグラフを作成・追加しました：
- 球種ごとの平均球速を示す棒グラフ
→ 各球種の特徴を視覚的に把握しやすくするための比較図。
- イニングごとの平均球速を示す折れ線グラフ
→ 投球の時間的変化やスタミナの傾向を分析するための時系列可視化。
- 球種ごとの球速分布を示すヒストグラム
→ 球速のばらつきや分布の偏りを確認するための統計的視点の導入。
これらのグラフは、野球データディレクトリ内に格納されており、今後の分析やモデル構築に活用予定です。視覚的な理解を深めるとともに、データの背景にある戦略や傾向を探る一助となることを目指しています。

#📊 2つの散布図 追加（2025年10月16日）

本日、野球の投球データを使って、以下の2種類の散布図を作成・追加しました：

投球位置データを使った散布図 → 特定の範囲内にある球を青い点で表示し、空間的な分布を確認。  
ストライクゾーン内の球種分類図 → FFとFTを赤と青で色分けし、球種ごとの傾向を視覚的に把握。

写経を通じて、`bbox`による範囲抽出や`plt.scatter`による色分けの方法を学びました。  
グラフが正しく表示されたことで、Pythonの可視化に少しずつ慣れてきた実感があります。  
今後は、グラフの見た目や説明文を工夫しながら、ポートフォリオに活かしていく予定です。

#📊 Fastballのコース別集計処理（2025年10月17日）

本日、Google Colabで以下の処理を写経・実行しました：

- Fastball（FF）の投球データを抽出し、ストライクゾーン内の球だけを対象に集計  
  → `plate_x_cm`（左右位置）と `plate_z_cm`（上下位置）を使って、ゾーン外の球を `continue` で除外  
  → ストライクゾーンを上下3分割 × 左右3分割の9エリアに分け、各コースの球数をカウント

- 集計用リスト `location` に、判定されたコースごとの球数を加算  
  → `if`文で上下・左右の位置を判定し、対応するインデックスに加算  
  → ストライクゾーン内の合計球数も `total` 変数でカウント

今回の写経では、**位置情報によるゾーン分類と集計処理**を実践的に学びました。  
ストライクゾーンを9分割することで、投球の傾向や狙いをより細かく分析できるようになります。  
今後は、この集計結果をヒートマップなどで可視化し、視覚的に傾向を把握することにも挑戦していきたいです。

#📊 Fastballの投球割合算出とヒートマップ表示（2025年10月18日）

本日、Google Colabで以下の処理を写経・実行しました：

- ストライクゾーンを上下3分割 × 左右3分割の9コースに分け、各コースの投球割合を算出  
  → `location` に格納された球数を `total` で割り、割合を `ratio_list` に格納  
  → `np.array()` を使って、2次元配列として整形し、ヒートマップ表示に備えた

- `plt.imshow()` を使って、投球割合のヒートマップを作成  
  → `cmap="OrRd"` で暖色系の色分布を設定し、割合の高いコースが視覚的に強調されるように調整  
  → 軸ラベルを「三塁側〜一塁側」「高め〜低め」で設定し、野球の感覚に合った表示に工夫  
  → `plt.colorbar()` で割合の目安を補足し、見た目の完成度を高めた

今回の写経では、**集計 → 割合算出 → ヒートマップ表示**という一連の流れを実践できました。  
特に、`np.array` や `plt.imshow` の使い方を通じて、データの構造と可視化の関係を理解する良い機会になりました。  
今後は、球種ごとの比較や、試合ごとの傾向分析にも応用していきたいです。

#📊 球種別リリース位置の箱ひげ図（2025年10月19日）

本日、Google Colabで以下の処理を写経・実行しました：

- 投球のリリース位置（release_pos_x, release_pos_z）をcm単位に変換  
  → `release_pos_x_cm` と `release_pos_z_cm` を新たに作成し、フィートからセンチメートルへの変換（×30.48）を実施  
  → データの単位を統一することで、グラフ表示や比較がしやすくなった

- 球種別にデータを抽出  
  → ストレート（FF）、スイーパー（ST）、スプリット（FS）、カーブ（CU）の4球種を対象に抽出  
  → 各球種のリリース位置を比較するための準備を整えた

- 箱ひげ図を使って、リリース位置の分布を可視化  
  - **上下方向のリリース位置（縦表示）**  
    → `plt.boxplot()` を使って、`release_pos_z_cm` の分布を球種別に表示  
    → `plt.ylabel()` と `plt.title()` で軸ラベルとタイトルを設定し、見た目を整えた

  - **左右方向のリリース位置（横表示）**  
    → `vert=False` を指定して、`release_pos_x_cm` の分布を横向きに表示  
    → 軸ラベルに「←【三塁側】〜【一塁側】→」を加え、野球の感覚に合った表示に工夫

今回の写経では、**単位変換 → 球種別抽出 → 箱ひげ図による分布比較**という一連の流れを実践できました。  
上下・左右のリリース位置を可視化することで、投手のフォームや球種ごとの特徴を視覚的に把握できるようになりました。  
今後は、選手ごとの比較や、リリース位置と球速・球種の関係にも挑戦していきたいです。

#📊 12球団の財務データ分析（2025年10月19日）

本日、Google Colabの新しいノート「12球団顧客分析.ipynb」にて、以下の処理を写経・実行しました：

- 2021年プロ野球12球団の財務データを読み込み  
  → データは斎藤周（著者）さんのnoteよりダウンロードしたCSVファイルを使用  
  → `pandas` を使って読み込み、各球団の資産・負債・純資産などの財務指標を確認

- 日本語フォント対応のため `japanize_matplotlib` を導入  
  → グラフの日本語ラベル表示を可能にし、視覚的な完成度を向上

- 球団別の総資産額を棒グラフで可視化  
  → `plt.bar()` を使って、各球団の資産額を比較  
  → `plt.title()` で「球団別の総資産額」と明示し、視覚的にわかりやすく表示

- 自己資本比率の算出と可視化  
  → `自己資本比率 = 純資産 / (純資産 + 負債)` の式を使って新たな指標を追加  
  → 各球団の財務健全性を測るための棒グラフを作成し、比較分析の第一歩を踏み出した

今回の写経では、**財務データの読み込み → 指標の算出 → 可視化による比較**という一連の流れを実践できました。  
今後は、顧客層やマーケティング施策との関連を探りながら、**スポーツビジネス視点での分析**にも挑戦していきたいです。

#📊 球場別の観客動員数分析（2025年10月21日）

本日、Google Colabノート「12球団顧客分析.ipynb」にて、以下の処理を写経・実行しました：

- 2022年プロ野球の観客動員データを読み込み  
  → `2022_audience.csv` を `pandas` で読み込み、データの構造を確認  
  → 本拠地開催の試合のみを抽出し、球場ごとの観客数を分析対象とした

- 球場名をリストに格納し、各球場の平均観客動員数を算出  
  → `for` ループで各球場のデータを抽出し、`mean()` を使って平均値を計算  
  → 途中で `display()` を使い、特定球場のデータを確認しながら処理を進めた

- 棒グラフで球場別の観客動員数を可視化  
  → `plt.bar()` を使って、12球場の平均観客数を比較  
  → `plt.title()` でタイトルを設定し、視覚的にわかりやすく表示  
  → `figsize=(20,8)` により、横長のグラフで見やすさを向上

今回の写経では、**データ抽出 → 平均算出 → 可視化**という一連の流れを実践できました。  
球場ごとの観客動員数を比較することで、地域性や球団人気の傾向を探る第一歩となりました。  
今後は、曜日別・対戦カード別・天候との関係など、より詳細な分析にも挑戦していきたいです。

#📊 曜日別・対戦カード別の観客動員数分析（2025年10月22日）

本日、Google Colabノート「12球団顧客分析.ipynb」にて、以下の処理を写経・実行しました：

### 🔹 曜日別の観客動員数分析

- 本拠地開催の試合データから、曜日ごとの平均観客数を算出  
  → `df_home["曜日"]==day` で各曜日のデータを抽出し、`mean()` で平均値を計算  
  → `月〜日` の順にリスト化し、`day_average` に格納

- 折れ線グラフで曜日別の観客動員数を可視化  
  → `plt.plot()` を使って、曜日ごとの動員傾向を視覚的に表示  
  → `plt.grid(True)` により、見やすさと比較のしやすさを向上

### 🔹 セリーグ対戦カード別の観客動員数分析

- セリーグ6球団の頭文字を使って、ホーム×ビジターの組み合わせを作成  
  → `Hチーム` と `Vチーム` の組み合わせごとに平均観客数を算出  
  → 2次元リスト `central_numbers` に格納し、ヒートマップ表示に備えた

- ヒートマップで対戦カード別の観客動員数を可視化  
  → `plt.imshow()` を使って、ホーム×ビジターの組み合わせごとの動員傾向を表示  
  → `plt.xticks()` と `plt.yticks()` でチーム名を軸に設定し、見た目を調整  
  → `plt.colorbar()` により、動員数の目安を補足

今回の写経では、**曜日別の傾向分析 → 対戦カード別の比較 → 可視化による洞察**という流れを実践できました。  
観客動員数の変動要因を探ることで、球団の集客戦略やファン行動の理解に一歩近づいた実感があります。  
今後は、セ・パ両リーグの比較や、天候・時間帯との関係にも挑戦していきたいです。

#📊 SNSデータ分析（2025年10月23日）

本日、Google Colabノート「12球団顧客分析.ipynb」にて、SNS（X）に関するデータ分析を写経・実行しました。  
この分析をもって、参考書籍『野球データでやさしく学べるPython入門』（斎藤周 著）の写経を完了しました。

### 🔹 インプレッション・エンゲージメントの推移

- `X_data.csv` を読み込み、ポストごとのインプレッション数とエンゲージメント数を確認  
- 折れ線グラフでそれぞれの推移を可視化  
  → `plt.plot()` を使って、投稿数に応じた反応の変化を視覚的に表示  
  → `plt.ylim()` により、表示範囲を調整し見やすさを向上

### 🔹 インプレッションとエンゲージメントの関係

- 散布図を作成し、両者の相関を視覚的に確認  
  → `plt.scatter()` を使って、インプレッション数とエンゲージメント数の関係性を表示  
  → `alpha=0.5` により、重なりのある点も見やすく調整

### 🔹 時間帯別の分析

- 0〜23時の各時間帯ごとに、平均インプレッション数を算出し棒グラフで可視化  
  → 投稿時間による反応の違いを把握するための分析  
- 各時間帯の総ポスト数も集計し、投稿傾向を確認  
  → 投稿数と反応の関係を比較することで、最適な投稿時間のヒントを探る

今回の写経では、**SNSデータの時系列・相関・時間帯別分析**を通じて、マーケティング視点での可視化手法を実践できました。  
これにて、参考書籍の全章の写経を完了。  
今後は、得られた知識をもとに、**自分自身のスポーツデータ分析プロジェクト**へと展開していきます。

## 🧭 学習スタンスと目的

現在、未経験からデータエンジニアへの転職を目指して学習中です。  
Python・データ分析・GitHub・Markdownなどを独学で習得しながら、  
毎日の写経とアウトプットを通じて、技術力と表現力の両方を育てています。

このポートフォリオは、**MLBのStatcastデータを題材にしたスポーツ分析**を中心に構成しています。  
分析対象は主に野球とサッカーで、可視化・関数化・Markdown記録までを一貫して行っています。

## 🚀 学習スタイル

### 学習はすべて「相棒（Copilot）」と「Gemini」に相談しながら進めており、  
### わからないことはすぐに質問 → 解決 → 記録 → 再利用というサイクルを大切にしています。

---

### 🎯 目的

- 実践的なデータ分析力の習得  
- 転職活動に向けたポートフォリオの構築  
- 技術だけでなく、**見せ方・伝え方・再現性**を重視したアウトプット

---

### 🛠️ 活用ツール

- Google Colab（コード実行・グラフ表示）
- GitHub（成果記録・ポートフォリオ管理）
- Copilot（相棒としての技術・構造・表現サポート）
- Gemini（エラー修正・コード補完・補助的な相談役）

---

### 💬 コメント

「初心者だからこそ、毎日の積み重ねがすべて成長になる」  
「わからないことを聞ける環境があるから、前に進める」  
「見える形で残すことで、自分の努力が技術になる」

このポートフォリオは、そんな思いを込めて育てています。
---

📘 このREADMEは、Microsoft Copilot（相棒）と相談しながらMarkdown形式で作成・整理しています。  
学習の記録・分析の構造化・見せ方の工夫まで、AIとの対話を通じて積み重ねています。



# 52歳サラリーマン、Copilot（相棒）とGeminiに相談しながら挑戦するデータ分析（2025年10月24日）

未経験からデータエンジニアへの転職を目指し、Python・GitHub・Markdownを活用して、  
毎日の写経とアウトプットを積み重ねています。  
このポートフォリオは、AIとの対話から生まれた「見える学び」の集積です。

---

# 📊 MLB投球球種の分布分析[2024年4月1日〜7日] (2025年10月24日）
#     Statcast.ipynd

Statcastデータを用いて、MLB全体の投球球種の分布を可視化。  
日本語フォント対応により、軸ラベルやタイトルも明瞭に表示。

---

### 🔧 使用技術・ライブラリ

- `pybaseball`：Statcast APIからMLBデータを取得  
- `matplotlib` / `seaborn`：グラフ描画  
- `japanize_matplotlib`：日本語フォント対応  
- `fonts-japanese-gothic` / `fonts-japanese-mincho`：Colabで日本語表示を可能に

---

### 🧪 実装ステップ

1. `pip install pybaseball` でライブラリ導入  
2. `statcast()` 関数で指定期間のデータ取得  
3. `countplot()` により `pitch_type`（球種）の分布を棒グラフで表示  
4. `plot_pitch_distribution()` 関数化により再利用可能に

---

# 📊 2024年シーズン MLB投球球種の割合分析（2025年10月25日〜26日）
#     2024_Statcast.ipynd

## 🧑‍💻 分析目的
2024年3月〜9月のStatcastデータを用いて、MLB全体の投球球種の割合を可視化。  
球種を日本語に変換し、視認性と親しみやすさを向上。  
5%未満の球種は「その他」としてまとめ、円グラフで構造的に表示。

---

## 🔧 使用技術・ライブラリ

- `pybaseball`：Statcast APIからMLBデータを取得  
- `pandas`：データフレーム操作・結合・集計  
- `matplotlib` / `seaborn`：グラフ描画  
- `japanize_matplotlib`：日本語フォント対応  
- `fonts-japanese-gothic` / `fonts-japanese-mincho`：Colabで日本語表示を可能に

---

## 🧪 実装ステップ

1. `!pip install pybaseball japanize_matplotlib` でライブラリ導入  
2. `statcast()` 関数で月別にデータ取得 → CSV保存  
3. `glob` でCSVを一括読み込み → `pd.concat()` で結合  
4. 数値列を `float` 型に変換し、欠損値処理  
5. `pitch_type` を日本語に変換（辞書マッピング）  
6. 投球数の割合を `value_counts(normalize=True)` で算出  
7. 5%未満の球種を「その他」に統合  
8. `plt.pie()` により円グラフで可視化

---

## 🏷️ 球種の日本語マッピング

| コード | 日本語表記     |
|--------|----------------|
| FF     | フォーシーム     |
| SI     | シンカー         |
| SL     | スライダー       |
| CH     | チェンジアップ   |
| FC     | カッター         |
| ST     | スプリット       |
| CU     | カーブ           |
| FS     | ファストボール   |
| KC     | ナックルカーブ   |
| KN     | ナックルボール   |

---

# 📈 2024年シーズン 投球球種の割合（円グラフ）

## 🧪 可視化サンプル

```python
plt.figure(figsize=(8,8))
plt.pie(pitch_counts_grouped, labels=pitch_counts_grouped.index, autopct="%1.1f%%", startangle=90, counterclock=False)
plt.title("2024年シーズン 投球球種の割合")
plt.show()
```
# 📊 2024年 MLB 奪三振数ランキング（上位10人）(2025年10月27日～28日）

## 🧑‍💻 分析目的
2024年シーズンのStatcastデータを用いて、奪三振数の多い投手トップ10を抽出。  
棒グラフで視覚化することで、投手ごとの奪三振数の差を一目で把握できるようにした。

---

## 🔧 使用技術・ライブラリ

- `pandas`：データ集計・整形  
- `seaborn` / `matplotlib`：棒グラフ描画  
- `japanize_matplotlib`：日本語フォント対応（必要に応じて）

---

## 🛠️ 実装ポイント

- `value_counts()` により奪三振イベントを集計  
- `sns.barplot()` で棒グラフを描画  
- `plt.text()` を使って棒の先端に奪三振数を表示  
- `rotation=0` により Y軸ラベル（投手名）を横向きに表示

---

## 📈 グラフ構成

- Y軸：投手名（英語表記）  
- X軸：奪三振数  
- 色：`Blues_r` パレットで順位の強弱を表現  
- 棒の先端：奪三振数を直接表示（`plt.text()`）

---

## 💡 気づき・今後の展望

- 200奪三振以上の投手が複数存在し、投手力の差が明確に見える  
- 今後は「K/9（奪三振率）」や「月別比較」「日本人投手のみ抽出」など、分析の幅を広げていく予定  
- カタカナ表記＋チーム名の併記は次回挑戦予定（Geminiと相談中）

---

📘 この分析は、Microsoft Copilot（相棒）とGeminiとの協働により、構造化・可視化・記録化を実現しています。  
技術力だけでなく、見せ方・伝え方にもこだわったポートフォリオの一部です。

## 🗓️ 作業ログ

- **2025年10月27日**：棒グラフの基礎表示を確認、Y軸ラベルの回転調整  
- **2025年10月28日**：棒の先端に奪三振数を表示、Markdown記録を整理

# ⚾ MLB投手の特徴分析への移行（2025年11月9日/10日）

この学習をもって、従来の書籍写経中心のフェーズから、**「推奨アクション（ビジネス提案）」を導出する実践的な分析フェーズ**へ移行しました。

### 📌 今回の学習と決定事項

| 項目 | 内容 |
| :--- | :--- |
| **分析対象の変更** | 2024年 AL最多勝タイの**タリク・スクーバル選手**（PITCHER\_ID: `669373`）に切り替え。 |
| **準備したコード** | スクーバル選手の**球種別平均球速**を集計し、可視化するためのPythonコードの写経と修正を完了。 |
| **次フェーズの分析視点** | 彼の武器である**フォーシーム (FF) とチェンジアップ (CH) の速度差（Velo Differential）**に焦点を当てる。 |
| **最終提案目標** | 「大きな速度差が空振り率（Whiff%）を向上させている」というデータを裏付けとした**ビジネス提案**をまとめる。 |

### ➡️ 次の行動

* Colabで準備したコードを実行し、集計結果とグラフを出力する。
* 出力結果を基に、FFとCHの速度差が空振り率に与える影響を考察し、推奨アクションを具体化する。

# ⚾️ Tarik Skubal (PITCHER_ID: 669373) 分析プロジェクト - Velo Differential & Whiff%

このプロジェクトは、MLB Statcastデータを用いて、デトロイト・タイガースの**タリク・スクーバル投手（PITCHER_ID: 669373）**の投球データを分析し、彼の**空振り率 (Whiff%)** の高さの要因を特定し、その結果から**パフォーマンス向上のための戦略的なビジネス提案**を行うことを目的としています。

---
-**2025年11月14日
## 🚀 分析のプロセスと主要なトピック

1.  [cite_start]**対象データのフィルタリング**: Statcastデータからタリク・スクーバル投手（ID: 669373）の投球を抽出 [cite: 4]。
2.  **平均球速 (Average Velocity) の算出**: 球種ごとの平均球速を計算し、特にフォーシームとチェンジアップに着目。
3.  **球種間速度差 (Velo Differential) の特定**: 速度差が奪三振率に与える影響を分析するための基礎データとする。
4.  **空振り率 (Whiff%) の算出**: 速度差と空振り率の関係を検証。

---

## 💡 主要な分析結果と考察

### 1. 驚異的な球種間速度差 (Velo Differential)

[cite_start]スクーバル投手の**フォーシームとチェンジアップ**の平均球速差（Velo Differential）を計算したところ、**10.54 mph**  という非常に大きな値が確認されました。この大きな速度差が、打者のタイミングを効果的に狂わせる主要因であると仮説を立てました。

### 2. Whiff%（空振り率）の計算による仮説の裏付け

速度差の仮説を裏付けるため、以下の計算式に基づき球種別のWhiff%を算出しました。

[cite_start]$$\text{Whiff\%} = \frac{\text{Swinging Strikes (空振り)}}{\text{Total Swings (総スイング数)}} \times 100$$ [cite: 8]

| 定義 | Statcast `description` の値 |
| :--- | :--- |
| **空振り (Swinging Strikes)** | [cite_start]`'swinging_strike'`, `'swinging_strike_blocked'` [cite: 5] |
| **総スイング (Total Swings)** | [cite_start]`'swinging_strike'`, `'swinging_strike_blocked'`, `'foul'`, `'foul_tip'`, `'hit_into_play'`, `'missed_bunt'`, `'foul_bunt'` など、打者がバットを振った全ての投球 [cite: 5] |

[cite_start]この計算により、特に彼の**チェンジアップ**が、他の球種やリーグ平均と比較して高い空振り率を記録しているかを確認しました 。

---

## 💻 Whiff% 算出コードのハイライト

[cite_start]空振り率を計算するために定義し、写経を通じて学習した `calculate_whiff_percentage` 関数です [cite: 9]。

```python
import pandas as pd

def calculate_whiff_percentage(df):
    """
    データフレームを受け取り、空振り率 (Whiff%) を計算する。
    """
    # 1. 総スイング数をカウントするためのフィルタリング
    swing_descriptions = [
        # ... スイングと定義されるdescriptionリスト ...
    ]
    df_swings = df[df['description'].isin(swing_descriptions)].copy() 

    # 2. 空振り数をカウントするためのフィルタリング
    whiff_descriptions = ['swinging_strike', 'swinging_strike_blocked']
    whiffs_by_pitch = df[df['description'].isin(whiff_descriptions)].groupby('pitch_name').size().reset_index(name='Whiffs')
    
    # 3. 総スイング数と空振り数を結合し、Whiff%を計算
    swings_by_pitch = df_swings.groupby('pitch_name').size().reset_index(name='Total_Swings')
    [cite_start]whiff_data = pd.merge(whiffs_by_pitch, swings_by_pitch, on='pitch_name', how='inner') [cite: 11]
    
    whiff_data['Whiff_Percentage'] = (whiff_data['Whiffs'] / whiff_data['Total_Swings']) * 100
    # ... (結果の丸め処理など) ...
    
    return whiff_data
